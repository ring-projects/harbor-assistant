...
import {
  getCodexSkillPreviewForProject,
  resolveCodexSkillsForProject,
...
type ProjectSkillPreviewPageProps = {
  params: Promise<{
    project_id: string
  }>
...
export default async function ProjectSkillPreviewPage(
  props: ProjectSkillPreviewPageProps,
) {
  ...
  const skillsResult = await resolveCodexSkillsForProject(project.path)
  ...
  const skillPreview = selectedSkill
    ? await getCodexSkillPreviewForProject({
                ...
                {skillsResult.skills.map((skill) => {
                  ...
                  return (
                    <li key={`${skill.source}:${skill.path}`}>
                      <Link
                        href={`/${projectId}/skills/preview?source=${skill.source}&name=${encodeURIComponent(skill.name)}`}
                        className={cn(
                        ...
                        {skillPreview.files.map((file) => {
                          ...
                          return (
                            <li key={file.absolutePath}>
                              <Link
                                href={`/${projectId}/skills/preview?source=${selectedSkill.source}&name=${encodeURIComponent(selectedSkill.name)}&file=${encodeURIComponent(file.relativePath)}`}
                                className={cn(
