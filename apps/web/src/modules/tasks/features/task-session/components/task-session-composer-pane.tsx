...
import { type TaskDetail, type TaskEffort } from "@/modules/tasks/contracts"
...
const TaskSessionComposerPaneContent = memo(
  function TaskSessionComposerPaneContent({
    detail,
    projectId,
    taskId,
  }: TaskSessionComposerPaneContentProps) {
    const draft = useTasksSessionStore(
      (state) => selectChatUi(state, taskId).draft,
    )
    ...
    const lastSequence = useTasksSessionStore((state) =>
      selectLastSequence(state, taskId),
    )
    ...
    const resolvedModelConfig = useMemo(() => {
      if (selectedModel) {
        return (
          selectedExecutorModels.find((model) => model.id === selectedModel) ??
          null
        )
      }
    ...
    return (
      <div className="min-h-0">
        <TaskInputComposer
          ...
          actionDisabled={
            detail?.status === "running" ? isCancelling : undefined
          }
