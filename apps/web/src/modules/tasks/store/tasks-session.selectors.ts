...
export function selectSelectedExecutionBlock(
  state: TasksSessionState,
  taskId: string | null,
): SelectedExecutionBlock | null {
  const executionBlockId = selectChatUi(state, taskId).selectedExecutionBlockId
  if (!executionBlockId) {
    return null
  }

  const block = selectConversationBlocks(state, taskId).find(
    (item): item is SelectedExecutionBlock =>
      item.type === "execution" && item.id === executionBlockId,
  )
