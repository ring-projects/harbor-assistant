...
import type { TaskDiff, TaskDiffFile, TaskDiffFileStatus, TaskDiffHunk } from "../types"
...
async function readGitStatus(projectPath: string) {
  const result = await runGitCommand(["status", "--porcelain=v1", "-z", "--untracked-files=all"], projectPath)
...
async function buildSyntheticUntrackedPatches(projectPath: string, statusEntries: GitStatusEntry[]) {
...
function finalizeFile(file: {
  ...
  return {
    ...
    hunks: file.hunks,
  } satisfies TaskDiffFile
}

export function parseUnifiedDiff(patchText: string): TaskDiffFile[] {
  if (!patchText.trim()) {
    return []
  }
...
export async function readTaskGitDiff(args: {
  taskId: string
  projectPath: string
}): Promise<TaskDiff> {
