...
1. `/`：Landing / 首次引导
2. `/:project_id`：项目主工作台（默认进入 progress 视图）
...
2. Project Shell 布局：
   ...
   - 右侧主内容区（Progress / Settings / 其他未来页面）
...
1. task 前端详细需求拆分为独立文档：`/Users/qiuhao/workspace/harbor-assistant/docs/frd-task-frontend.md`
...
1. `/api/projects`（列表、创建）
2. `/api/projects/:id`（更新、删除）
3. `/api/fs/list`（目录浏览）
4. `/api/capabilities`（执行器能力探测）
5. Server Actions（`tasks`, `projects`, `codex-mcp`）
...
1. 关键事件埋点：
   ...
   - `task_cancel_clicked`
   - `task_retry_clicked`
2. 每次任务创建、取消、重试操作必须带 `projectId` 与 `taskId`（若可用）。
